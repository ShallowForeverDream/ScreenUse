...
| 视频/媒体 | VLC、mpv、PotPlayer、MPC、Windows Media Player，以及哔哩哔哩、YouTube、爱奇艺、优酷、腾讯视频等常见视频网站 | 当前媒体标题；依次读取网页 `videoPlaying`、Windows 系统媒体会话和播放控件。浏览器不公开这些状态时，仅对明确的前台视频播放页使用保守兜底；检测到暂停后恢复 3 分钟空闲规则 |
...
- 系统媒体会话只在即将达到空闲阈值时探测，并缓存 5 秒，避免为普通浏览持续扫描播放状态。
