...
- 除确认正在播放的网页视频或本地视频播放器外，其他应用 3 分钟没有键鼠输入即回溯为“离开”。
...
1. 确认正在播放的视频保持有效；其余活动命中空闲阈值时回溯归为“离开”。
...
- AI 只处理低于 80% 且至少 1 分钟的已结束会话；单次最多批量复核 8 条，每条携带前后 30 分钟时间段以及当前全部分类、项目和任务。模型只能选择已有层级，结果原位更新，不制造重叠时间段。
