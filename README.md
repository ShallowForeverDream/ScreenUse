...
修正时可以直接新建或删除分类、项目和任务，并选择“记住规则”或“固定 30 分钟”。分类、项目和任务都支持键入搜索、方向键选择和 Enter 确认；没有匹配项时 Enter 直接创建。任务可跨项目搜索，项目可跨分类搜索，同名项会带父级信息分别显示。相似活动以后会优先命中已确认的上下文，而不是看到 `ChatGPT.exe` 就固定归到某个开发项目。
