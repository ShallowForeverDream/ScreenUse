...
首次升级会建立 `休息 → 睡眠 → 午睡 / 睡觉` 层级，并从升级当天开始计算，避免把已有历史误算成多年欠眠。每天以 8 小时为基准：不足部分在第一层逐日增加 50%，每周一另加 3 小时不参与逐日增长；超过 8 小时的部分按最早欠缺优先偿还。第一层欠缺满 7 天仍未补足时进入第二层，之后每 7 天增加 100%，且额外睡眠对第二层只有三分之一的偿还效果。首页“睡眠缺失”卡显示两层当前合计；点击可查看最近 53 周睡眠热力图，再点击日期查看当日目标、缺失和每个睡眠时间段。跨午夜的睡眠时间会按本地自然日拆分，重叠记录不会重复计算。
